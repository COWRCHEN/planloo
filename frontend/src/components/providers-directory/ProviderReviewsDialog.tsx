...
import { useProviderReviews, type RatingBreakdown } from '@/hooks/use-providers';
...
export function ProviderRatingBreakdownBars({ breakdown, total }: RatingBreakdownBarsProps) {
  return (
...
export function ProviderReviewsDialog({
  ...
  const { data, isLoading } = useProviderReviews(
    providerUuid,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[80vh] flex flex-col gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle className="text-base">Reviews for {providerName}</DialogTitle>
          {/* Summary row */}
          <div className="flex items-start gap-4 pt-3">
            ...
            {data?.breakdown && (
              <div className="flex-1">
                <ProviderRatingBreakdownBars breakdown={data.breakdown} total={ratingCount} />
              </div>
            )}
          </div>
        </DialogHeader>
