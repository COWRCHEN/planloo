/**
 * Event Recent Activity Component
 *
 * Displays a unified activity feed scoped to a single event,
 * aggregated from tasks and guests belonging to that event.
 */

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { useEventActivity, type ActivityType } from '@/hooks/use-dashboard';
...
export function EventRecentActivity({ eventUuid }: EventRecentActivityProps) {
  const { data: activities, isLoading, error } = useEventActivity(eventUuid, 8);
            ...
            {activities.map((activity, i) => {
              ...
              const href =
                activity.type === 'task_created' || activity.type === 'task_completed' || activity.type === 'template_applied'
                  ? `/dashboard/events/${eventUuid}/tasks`
                  : activity.type === 'guest_added'
                    ? `/dashboard/events/${eventUuid}/guests`
                    : `/dashboard/events/${eventUuid}`;
